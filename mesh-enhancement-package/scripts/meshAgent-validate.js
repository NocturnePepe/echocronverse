...
async function validateMeshSystem() {
  let allPassed = true;
  ...
  if (fs.existsSync('.devcontainer.json')) {
    ...
    try {
      ...
      if (devconfig.postStartCommand && (devconfig.postStartCommand.includes('meshAgent') || devconfig.postStartCommand.includes('mesh:'))) {
  ...
  try {
    // Import the quick agent module dynamically
    await import('./meshAgent-quick.js');
...
// Run the validation
validateMeshSystem().then(allPassed => {
  if (allPassed) {
